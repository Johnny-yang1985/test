...
• For S32K310, S32K311, and S32K312: DMAMUX_0 channels 0-5 and DMAMUX_1 channels 0-5 are 
mapped to eDMA Transfer Control Descriptor(TCD) 0-5 and eDMA Transfer Control Descriptor(TCD) 6-11, 
respectively. Programming of DMAMUX_0 channels 6-15 and DMAMUX_1 channels 6-15 is not therefore 
expected however if programmed then any access will terminate with either error response for channels 8-15 
or without error response for channels 6-7.
...
Table 52. PIT channel assignment for periodic DMA triggering through DMAMUX
DMAMUX0/1 ports
PIT channel
pit_dma_trigger[0]
PIT0 channel 0
...
trigger event is observed (see Figure 32).
NXP Semiconductors
Direct Memory Access Multiplexer (DMAMUX)
S32K3xx Reference Manual, Rev. 10, 04/16/2025
Reference Manual
...
Trigger
DMA request
Figure 32. DMAMUX channel triggering: normal operation
...
requesting a transfer, then that trigger is ignored (see Figure 33).
Peripheral request
Trigger
DMA request
Figure 33. DMAMUX channel triggering: ignored trigger
...
14.4.2.1
Enable a source with periodic triggering
...
CHCFGn[ENBL] and CHCFGn[TRIG] are 1.
...
CHCFGn[ENBL] is 1 while CHCFGn[TRIG] is 0.
...
Reset value
0h - Fh
Channel Configuration (CHCFG0 - CHCFG15) 1
8
RW
00h
NXP Semiconductors
...
description.
14.5.1.2
Channel Configuration (CHCFG0 - CHCFG15)
Offset
For n = 0 to 15:
Register
Offset
...
if a channel is disabled (CHCFGn[ENBL] == 0).
...
Enables the periodic triggering capability for the triggered DMA channel.
...
14.6 Glossary
Slot
DMA source that either reflects a peripheral source or an always-enabled source
Periodic trigger
Trigger that an external source generates at periodic intervals for DMA transfer
NXP Semiconductors
Direct Memory Access Multiplexer (DMAMUX)
S32K3xx Reference Manual, Rev. 10, 04/16/2025
Reference Manual
