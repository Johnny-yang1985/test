...
processing
S32K3xx
Figure 235. Block diagram
Table 333. Block diagram components
Component
Description
ADC
Measures external analog signals
BCTU
Performs advanced ADC triggering
Control processing
Arm Cortex-M7 core:
...
LCU
Performs programmable logic functions and fault control
LPCMP
Compares analog values to detect faults
TRGMUX
...
ADCn Result Data (ADCnDR) register for the CPU to read.
BCTU-to-ADC interface illustrates the connections between BCTU and an ADC.
See also:
• Body cross-triggering unit (BCTU)
• PCDRn, ICDRn, and ECDRn register descriptions in ADC register descriptions
...
TRGMUX
TRGMUX
Figure 236. BCTU-to-ADC interface
NXP Semiconductors
Power Conversion and Motor Control (PCMC)
S32K3xx Reference Manual, Rev. 10, 04/16/2025
Reference Manual
...
eMIOS generates high-resolution PWM outputs. Each eMIOS instance has 24 dedicated channel outputs. You can configure each 
...
LCU has three force inputs that you can use to manage fault assertion via its own internal logic mechanism.
...
Figure 239. Portion of TRGMUX connectivity
See also:
• Trigger Mux (TRGMUX)
• TRGMUX connectivity file attached to this document
NXP Semiconductors
...
59.3.9.1
PCMC connections diagram
ADC0-2
IO MUX
CORE_CLK
ADC mux outputs
ADC std/prec/ext inputs
BCTU
next command
end of conversion
conversion data
...
eMIOS_n_ipp_ind_emios_ch[ ]
ADC12_n_EXTRG for normal conversion
ADC12_n_EXTRG for injected conversion
ADC12_n_EXTRG for sync conversion
T
R
G
M
...
X
AIPS_SLOW_CLK
SIUL2_TRGMUX_OUT0-15 (IO MUX)
eMIOS_n_reload_out_ch[ ]
eMIOS_n_ipp_do_emios_ch[ ]
LCU0-1
IO MUX
LCUn_OUT[0:11]
CORE_CLK
...
eDMA, FlexIO, LPI2C,
LPSPI, LPUART, PIT, LPCMP
SIUL_TRGMUX_IN0-15 (IO MUX)
BCTU_Trg23,47,71
Figure 240. PCMC connections diagram
...
59.5.1.1
PMSM motor control use case
59.5.1.1.1
Generic PMSM motor control
...
Motor control
logic
Figure 241. Generic PMSM motor control
Table 334. Block diagram components
Block
Description
Angular position sensor
Generates data that enables commutation logic to calculate rotor position.
Brushless DC motor
Electronically commutated electric motor.
Commutation logic
Compares values from the rotor angular position sensors to entries in a lookup 
...
Block
Description
DC-to-3-phase inverter
Switching network that transmits power to the motor stator windings according to a 
control signal.
Motor control logic
...
Generates PWM signal voltage source.
Reference signal
Signal that indicates required motor speed.
Speed and back EMF measurement
An ADC is linked to a shunt resistor to perform measurements and detect over-
...
Figure 242. PMSM motor control
Table 335. Block diagram components
Block
Description
ADC
Measures analog inputs (currents and/or voltages) and converts to digital values. 
...
BCTU
Triggers conversions by ADC.
Control process
Provides top-level system control functions for FOC or another feasible 
control algorithm:
Table continues on the next page...
NXP Semiconductors
Power Conversion and Motor Control (PCMC)
...
Table 335. Block diagram components (continued)
Block
Description
• Compares speed measurement to required speed and adjusts the PWM 
signal generator.
...
LCU
Implements commutation lookup table. Control Process compares values from the 
...
59.5.1.2
Sensored BLDC motor control use case
59.5.1.2.1
Generic sensored BLDC motor control
...
Motor control
logic
Figure 243. Generic sensored BLDC motor control
Table 336. Block diagram components
Block
Description
Angular position sensor
Hall effect sensors or optical decoders detect rotor position
Commutation logic
Compares values from the angular position sensors to entries in a lookup table and 
...
Table 336. Block diagram components (continued)
Block
Description
DC-to-3-phase inverter
Switching network that transmits power to the stator windings according to a 
control signal
Motor control logic
• Compares speed measurement to required speed and adjusts the PWM 
signal generator
...
commutation logic input
Speed measurement
Uses an ADC linked to a shunt resistor to perform measurements and detect 
over-current conditions, in which case hardware generates an interrupt.
59.5.1.2.2
PCMC sensored field-oriented control (FOC) of 3-phase BLDC motor
...
implemented in
software or firmware
Figure 244. PCMC sensored BLDC motor control
Table 337. Block diagram components
Block
Description
ADC
Measures analog inputs (currents and/or voltages) and converts to digital values. 
...
Triggers conversions by ADC.
Control process
Provides top-level system control functions for FOC control:
Table continues on the next page...
NXP Semiconductors
Power Conversion and Motor Control (PCMC)
S32K3xx Reference Manual, Rev. 10, 04/16/2025
...
Table 337. Block diagram components (continued)
Block
Description
• Compares speed measurement to required speed and adjusts the PWM 
signal generator.
...
Table 338. Example ADC configurations (continued)
Description
Settings
BCTU trigger mode
• Enable Overwrite (OWREN=1)
...
59.6.3.3
eMIOS channel modes useful in motor control
Table 339. eMIOS channel modes useful in motor control
Mode
Description
Use in motor control
MCB
...
Position measurement
OPWMCB
Center aligned Output PWM Buffered with dead time PWM duty cycle
OPWMB
Output Pulse Width Modulation Buffered
Trigger placement within the period
OPWMT
Output Pulse Width Modulation Trigger
Trigger placement within the period
59.6.3.3.1
MCB PWM and trigger generation
...
Channel 0 - MCB up down
Modulo (channel 0 An register) is double-buffered
Duty cycle is double-buffered (An registers for channels 
Channel 1- OPWMCB
Channel 2 - OPWMCB
Channel 3 - OPWMCB
Channel 1 An
...
Channel 2 An
Channel 3 An
1, 2, and 3) and updated after a reload signal.
and updated after a reload signal.
Figure 245. MCB PWM trigger generation
59.6.3.3.2
OPWMCB PWM and trigger generation
...
Channel 0 An
Modulo (channel 0 An register) is double-buffered
Duty cycle is double-buffered (An and Bn registers
Channel 1- OPWMCB
Channel 2 - OPWMCB
Channel 3 - OPWMCB
Channel 0
...
Reload
and updated after a reload signal.
 of channels 1-4) and updated after a reload signal.
Figure 246. OPWMCB PWM trigger generation
59.7 Glossary
BLDC
Brushless DC. An electronically commutated DC motor.
CF
Crest Factor. A waveform parameter that is the ratio of peak values to its RMS value.
DF
Distortion Factor.
NXP Semiconductors
Power Conversion and Motor Control (PCMC)
...
Displacement Power Factor In power electronics, the phase relationship between voltage and current: cos(θv – θi), where θv 
is the phase of the voltage and θi is the phase of the current.
DVA
Distortion Volt-Amps.
FF
Form Factor.
FOC
...
Gain Margin.
pf
Power factor. In power electronics, it is the ratio of average power to RMS power.
PMSM
